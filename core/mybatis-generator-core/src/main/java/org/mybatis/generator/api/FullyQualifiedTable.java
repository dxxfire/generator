...
public class FullyQualifiedTable {
   ...
   *          the actual catalog of the table as returned from DatabaseMetaData.
   *          This value should only be set if the user configured a catalog.
   *          Otherwise the DatabaseMetaData is reporting some database default
   *          that we don't want in the generated code.
   * 
   * @param introspectedSchema
   *          the actual schema of the table as returned from DatabaseMetaData.
   *          This value should only be set if the user configured a schema.
   *          Otherwise the DatabaseMetaData is reporting some database default
   *          that we don't want in the generated code.
   * 
   ...
   *          the configured domain object name for this table. If nothing is
   *          configured, we'll build the domain object named based on the
   *          tableName or runtimeTableName.
   * 
   * @param alias
   *          a configured alias for the table. This alias will be added to the
   *          table name in the SQL
   * 
   * @param ignoreQualifiersAtRuntime
   *          if true, then the catalog and schema qualifiers will be ignored
   *          when composing fully qualified names in the generated SQL. This is
   *          used, for example, when the user needs to specify a specific
   *          schema for generating code but does not want the schema in the
   *          generated SQL
   * 
   * @param runtimeCatalog
   *          this is used to "rename" the catalog in the generated SQL. This is
   *          useful, for example, when generating code against one catalog that
   *          should run with a different catalog.
   * 
   * @param runtimeSchema
   *          this is used to "rename" the schema in the generated SQL. This is
   *          useful, for example, when generating code against one schema that
   *          should run with a different schema.
   * 
   * @param runtimeTableName
   *          this is used to "rename" the table in the generated SQL. This is
   *          useful, for example, when generating code to run with an Oracle
   *          synonym. The user would have to specify the actual table name and
   *          schema for generation, but would want to use the synonym name in
   *          the generated SQL
   * 
